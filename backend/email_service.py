...
Service 24/7 | +33 06 05 90 61 63
        ...
        # Envoyer l'email
        # Créer la connexion SMTP avec configuration IONOS spécifique
        smtp = aiosmtplib.SMTP(hostname=SMTP_CONFIG["hostname"], port=SMTP_CONFIG["port"])
        await smtp.connect()
        await smtp.starttls()  # Démarrer TLS après connexion
...
async def send_confirmation_email(contact_data: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        # Créer la connexion SMTP pour email de confirmation
        smtp = aiosmtplib.SMTP(hostname=SMTP_CONFIG["hostname"], port=SMTP_CONFIG["port"])
        await smtp.connect()
        await smtp.starttls()  # Démarrer TLS après connexion
        await smtp.login(SMTP_CONFIG["username"], SMTP_CONFIG["password"])
